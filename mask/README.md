...
### 核心操作与工具
1. **双屏同步对比 (Dual View Mode)**：
   - 勾选右侧侧边栏或点击工具栏 🖼️ 按钮（快捷键 `d`）开启**双屏左右对比**；
   - 左屏显示 Mask 掩码与交互编辑工具，右屏显示 100% 纯荧光原图；
   - 无论在左屏还是右屏进行**滚轮放大/缩小**或**按住平移**，双屏均进行**像素级 1:1 实时同步联动**！
2. **缩放与平移 (Zoom & Pan)**：
...
4. **点选与侧边栏勾选 (Checkbox List)**：
   - 👆 模式下，左键点击 Canvas 上的细胞即可快速切换 **保留 (绿色)** / **剔除 (红色)**；
   - 右侧侧边栏提供完整细胞列表与 `🔍 搜索框`，可直接勾选/取消勾选；
