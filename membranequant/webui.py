...
def run_from_ui(
    ...
    try:
        ...
        report = (
            ...
            f"{warn_qc}\n"
            f"**输出说明：**\n"
            f"- `csv/results.csv`：单细胞明细（**RatioOfMeans_T_R** [主终点]、**Enrichment_Membrane_vs_Whole** [辅助终点]、**Ratio_T_over_R** 等）\n"
            f"- `csv/summary.csv`：按条件汇总（默认主指标 **RatioOfMeans_T_R**；同时输出 `summary_Enrichment.csv` 辅助指标）\n"
